...
bool SpritesheetAnimation::Create(sf::Sprite *sprite,
                                  const Vector2ui &frameSize,
                                  const Vector2ui &start, const int frameCount,
                                  const float frameInterval, bool repeat) {
...
void SpritesheetAnimation::Play() { m_playing = true; }

void SpritesheetAnimation::Pause() { m_playing = false; }

void SpritesheetAnimation::Reset() {
    m_playing = false;
    m_timer = 0;
    m_sprite->setTextureRect(
        sf::IntRect(m_start.x, m_start.y, m_frameSize.x, m_frameSize.y));
}
